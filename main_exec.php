...
switch ($_REQUEST['exec'])
{
	...
	case "create_movie" :
		...
		$out_exec = 'ffmpeg \\-loop 1 -t 1 -i ./files/F50C0DFE82/medium/final_F50C0DFE82_1.jpg \\-loop 1 -t 1 -i ./files/F50C0DFE82/medium/final_F50C0DFE82_2.jpg \\-loop 1 -t 1 -i ./files/F50C0DFE82/medium/final_F50C0DFE82_3.jpg \\-loop 1 -t 1 -i ./files/F50C0DFE82/medium/final_F50C0DFE82_4.jpg \\-loop 1 -t 1 -i ./files/F50C0DFE82/medium/final_F50C0DFE82_4.jpg \\';
		$out_exec .= "-filter_complex \\";
		$out_exec .= '"[1:v][0:v]blend=';
		$out_exec .= "all_expr='A*(if(gte(T,0.5),1,T/0.5))+B*(1-(if(gte(T,0.5),1,T/0.5)))'[b1v]; \\";
		$out_exec .= " [2:v][1:v]blend=all_expr='A*(if(gte(T,0.5),1,T/0.5))+B*(1-(if(gte(T,0.5),1,T/0.5)))'[b2v]; \\";
		$out_exec .= " [3:v][2:v]blend=all_expr='A*(if(gte(T,0.5),1,T/0.5))+B*(1-(if(gte(T,0.5),1,T/0.5)))'[b3v]; \\";
		$out_exec .= " [4:v][3:v]blend=all_expr='A*(if(gte(T,0.5),1,T/0.5))+B*(1-(if(gte(T,0.5),1,T/0.5)))'[b4v]; \\";
		$out_exec .= " [0:v][b1v][1:v][b2v][2:v][b3v][3:v][b4v][4:v]concat=n=9:v=1:a=0,format=yuv420p[v]";
		$out_exec .= ' " -pix_fmt yuv420p -map "[v]" ./files/F50C0DFE82/medium/out.mp4';



		exec(stripslashes($out_exec));
