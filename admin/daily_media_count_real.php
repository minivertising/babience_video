          ...
          <div class="table-responsive">
            ...
            <div id="daily_applicant_count_div1" style="display:block">
              <table class="table table-hover">
                ...
                <tbody>
<?php
	$daily_date_query	= "SELECT mb_regdate FROM ".$_gl['member_info_table']." WHERE mb_regdate not like '%2016-01-31%' Group by substr(mb_regdate,1,10) ORDER BY mb_regdate DESC";
	$date_res			= mysqli_query($my_db, $daily_date_query);
	while($date_daily_data = mysqli_fetch_array($date_res))
	{
		$daily_date		= substr($date_daily_data['mb_regdate'],0,10);
		$media_query	= "SELECT mb_media, COUNT( mb_media ) media_cnt FROM ".$_gl['member_info_table']." WHERE 1 AND mb_name<>'admin' AND mb_regdate LIKE  '%".$daily_date."%' AND mb_lms='Y' GROUP BY mb_media";
		...
		while ($media_daily_data = mysqli_fetch_array($media_res))
		{
			...
			$pc_query		= "SELECT * FROM ".$_gl['member_info_table']." WHERE 1 AND mb_name<>'admin' AND mb_regdate LIKE  '%".$daily_date."%' AND mb_media='".$media_daily_data['mb_media']."' AND mb_lms='Y' AND mb_gubun='PC'";
			$pc_count		= mysqli_num_rows(mysqli_query($my_db, $pc_query));
			$mobile_query	= "SELECT * FROM ".$_gl['member_info_table']." WHERE 1 AND mb_name<>'admin' AND mb_regdate LIKE  '%".$daily_date."%' AND mb_media='".$media_daily_data['mb_media']."' AND mb_lms='Y' AND mb_gubun='MOBILE'";
